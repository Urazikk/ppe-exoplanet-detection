...
def login():
    ...
    return jsonify({"token": token, "username": username, "expires_in": TOKEN_EXPIRY})
...
# ============================================
# ANALYSE AVEC PROGRESSION SSE (Server-Sent Events)
# ============================================
@app.route('/api/analyze/stream', methods=['GET'])
def analyze_stream():
    """
    Endpoint SSE : envoie la progression en temps reel puis le resultat final.
    Format des events:
      - event: progress  -> data: {"step": "...", "message": "...", "percent": N}
      - event: result    -> data: { ... resultat complet ... }
      - event: error     -> data: {"error": "..."}
    """
    # Auth manuelle (pas de decorateur pour SSE)
    auth_header = request.headers.get('Authorization', '')
    if not auth_header.startswith('Bearer '):
        return jsonify({"error": "Token manquant"}), 401
    token = auth_header.split(' ')[1]
    ...
    def generate():
        def send_progress(step, message, percent=None):
        ...
        def send_result(data):
            yield f"event: result\ndata: {json.dumps(data, allow_nan=False, default=str)}\n\n"

        # Cache check
        if target_id in _analysis_cache and not force_refresh:
            yield from send_progress("cache", "Resultat en cache.", 100)
            yield from send_result(_analysis_cache[target_id])
            return

        try:
            # STEP 1 : Acquisition (20%)
            yield from send_progress("acquisition", f"Telechargement des donnees pour {target_id}...", 5)
            
            mission = "TESS" if any(x in target_id for x in ["TIC", "TOI", "Pi", "WASP"]) else "Kepler"
            lc_raw = fetch_lightcurve(target_id, mission=mission)

            if lc_raw is None:
                yield from send_error("Cible introuvable dans les archives NASA")
                return

            n_raw = len(lc_raw)
            yield from send_progress("acquisition", f"{n_raw:,} points telecharges.", 20)

            # STEP 2 : Preprocessing (40%)
            yield from send_progress("preprocessing", f"Nettoyage et binning de {n_raw:,} points...", 25)
            
            # Callback pour progression du preprocessing
            preprocess_msgs = []
            def on_preprocess(step, msg):
                preprocess_msgs.append(msg)
            
            lc_clean = clean_and_flatten(lc_raw, quality="fast", progress_cb=on_preprocess)

            if lc_clean is None:
                yield from send_error("Echec du preprocessing")
                return

            yield from send_progress("preprocessing", f"Preprocessing OK ({len(lc_clean):,} points).", 40)

            # STEP 3 : BLS (60%)
            yield from send_progress("bls", "Recherche de periode (BLS 2 passes)...", 45)
            
            bls_msgs = []
            def on_bls(step, msg):
                bls_msgs.append(msg)
            
            period = get_period_hint(lc_clean, progress_cb=on_bls)
            
            lc_folded = fold_lightcurve(lc_clean, period=period)
            period_val = safe_float(period) or 0.0

            yield from send_progress("bls", f"Periode detectee : {period_val:.4f} jours.", 60)

            # STEP 4 : Feature extraction + prediction (90%)
            yield from send_progress("prediction", "Extraction TSFRESH + prediction XGBoost...", 65)
            ...
            if model and selected_features:
                ...
                if features_df is not None:
                    yield from send_progress("prediction", "Features extraites. Prediction en cours...", 80)
            ...
            yield from send_progress("prediction", f"Score IA : {score*100:.1f}%", 90)

            # STEP 5 : Formatage (100%)
            yield from send_progress("formatting", "Preparation des donnees graphiques...", 95)
            ...
            for t, f in zip(lc_folded.time.value[::step], lc_folded.flux.value[::step]):
                ...
                if t_val is not None and f_val is not None:
                    chart_data.append({"time": round(t_val, 4), "flux": round(f_val, 6)})
            ...
            yield from send_progress("done", "Analyse terminee.", 100)
            yield from send_result(result)
...
# ============================================
# ENDPOINT CLASSIQUE (fallback non-SSE)
# ============================================
    ...
    try:
        ...
        if model and selected_features:
            ...
            if features_df is not None:
                ...
                score = safe_float(raw_score) or 0.5
                importances = model.feature_importances_
                feat_imp = sorted(zip(selected_features, importances), key=lambda x: x[1], reverse=True)[:5]
                top_features_list = [{"name": n, "importance": round(float(i), 4)} for n, i in feat_imp]

        step = max(1, len(lc_folded) // 1000)
        chart_data = []
        for t, f in zip(lc_folded.time.value[::step], lc_folded.flux.value[::step]):
            t_val, f_val = safe_float(t), safe_float(f)
            if t_val is not None and f_val is not None:
                chart_data.append({"time": round(t_val, 4), "flux": round(f_val, 6)})

        period_val = safe_float(period) or 0.0
        result = {
            "target": target_id, "mission": mission, "score": score,
            "period": round(period_val, 4), "points_count": len(lc_raw),
            "top_features": top_features_list, "data": chart_data
        }
        _analysis_cache[target_id] = result
...
def validate_target():
    ...
    if target_id not in NASA_CATALOG:
        return jsonify({"target": target_id, "in_catalog": False, "message": "Cette etoile n'est pas dans notre catalogue de validation."})
    ...
    if target_id in _analysis_cache:
        ...
        return jsonify({
            "target": target_id, "in_catalog": True,
            "prediction_score": score, "predicted_planet": predicted_planet,
            "nasa_confirmed": actual_planet, "correct": predicted_planet == actual_planet,
            "planet_name": truth.get("planet_name"), "known_period": truth.get("period_days"),
        })
    return jsonify({"target": target_id, "in_catalog": True, "message": "Analysez d'abord cette etoile avant de valider."})
